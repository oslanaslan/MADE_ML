...
def test_dump_valid_data_to_pickle(tmp_path):
    """Test dump to pickle with valid data"""
...
def test_load_read_invalid_dict_from_pickle(tmp_path):
    """Test load with invalid data from pickle"""
    ...
    with open(invalid_data_path, 'wb') as file:
...
def test_load_result_type_from_pickle(tmp_path):
    """Test load function output from pickle"""
...
# StoragePolicy & JsonPolicy & PicklePolicy tests
...
def test_json_and_pickle_instance_of_storage_policy():
    """JsonPolicy and PicklePolicy must be instance of StoragePolicy"""

    assert issubclass(JsonPolicy, StoragePolicy), (
        f"JsonPolicy must be instace of StoragePolicy\n\
            Got: {issubclass(JsonPolicy, StoragePolicy)}\n"
    )
    assert issubclass(PicklePolicy, StoragePolicy), (
        f"JsonPolicy must be instace of StoragePolicy\n\
            Got: {issubclass(PicklePolicy, StoragePolicy)}\n"
    )
...
def test_pickle_policy_dump(tmp_path):
    """Test dump to pickle with valid data"""
    ...
    valid_dict = defaultdict(list, valid_dict)
    PicklePolicy.dump(valid_dict, file_path)
...
def test_pickle_policy_load(tmp_path):
    """Test dump from pickle with valid data"""
    ...
    inverted_index = PicklePolicy.load(file_path)
